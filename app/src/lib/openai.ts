...
export async function generateImage({
  ...
  characterAttributes,
  prompt
}: ImageGenerationParams): Promise<{ imageUrl: string, imageDescription: string }> {
  try {
    ...
    if (characterType === 'baby') {
      ...
      imagePrompt = prompt || `Generate a high-quality, photorealistic portrait of ${characterDescription} in a professional podcast studio with headphones and microphone. The character should be centered in the frame with good lighting.`;
    }
    ...
    if (result.data[0].b64_json) {
      ...
      throw new Error('No image data returned from OpenAI in a recognized format');
    }
...
export async function generateScript({
  ...
  try {
    ...
    return completion.choices[0].message.content || "Failed to generate script content.";
  } catch (error) {
